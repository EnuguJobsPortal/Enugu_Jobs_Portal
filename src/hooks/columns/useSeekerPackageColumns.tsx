...
import { ISeekerPackageResponse } from "@/interfaces/response.interface";
...
const useSeekerPackageColumns = ({ handleDeletePackage }:ITableColumnProps) => {
    const { pagination, onPageChange } = usePagination({ pageIndex:0, pageSize:10 });
    const [ pkg, setPkg ] = useState<ISeekerPackageResponse>();
    const [ showUpdateModal, setShowUpdateModal ] = useState<boolean>(false);

    const seekerPackageColumns: ColumnDef<ISeekerPackageResponse>[] = [
		{
			accessorKey: '',
			header: 'SNO',
		...
		},
		{
			...
			cell: ({ row }) => {
                ...
                const menuItems: ITableCellMenuItem[] = [
                    ...
                    { label: 'Delete', icon: 'Trash2', iconColor: '#6b7280', handleClick: () => { handleDeletePackage(row.original.TrainingPackageId) }},
                ];
...
export default useSeekerPackageColumns;