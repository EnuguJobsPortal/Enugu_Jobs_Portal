...
const NewJobPost = () => {
    const [ jobPostResponse, setJobPostResponse ] = useState<INewJobPostResponse>()
    ...
    const { isLoading, users } = useGetAllUsers({ params: { UserTypeId: "2"} });
    ...
    const { verifyPayment, verifyingPayment } = useVerifyJobPostPayment({ onSuccess: (values) => {
        hideLoading();
    ...
    const userOptionsEnum: SelectOption[] = users ? users?.data.map((user) => (
    ...
    }, [ jobPostResponse, initializePayment ])
